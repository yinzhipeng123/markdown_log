...
```sql
mysql> select max(ule),min(ula) from score;
+----------+----------+
| max(ule) | min(ula) |
+----------+----------+
|       90 |       45 |
...
```sql
mysql> select max(ule) mule,min(ula) as mula from score;  //给列起别名
+------+------+
| mule | nula |
+------+------+
|   90 |   45 |
...
mysql> select count(sno) from score;       //忽略空值
+------------+
| count(sno) |
+------------+
|          4 |
+------------+
1 row in set (0.00 sec)
mysql> delete from score where sno is null;     //将sno字段为空的行删除掉
is null：用来匹配空的
is not null：匹配非空的
```
...
```sql
mysql> select sum(ule),sum(ula),sum(uoa) from score;
+----------+----------+----------+
| sum(ule) | sum(ula) | sum(uoa) |
+----------+----------+----------+
|      290 |      300 |      336 |
+----------+----------+----------+
1 row in set (0.00 sec)
...
```sql
mysql> select sname,ule+ula+uoa total from score;    //select后面可以加算数表达式
+-------+-------+
| sname | total |
+-------+-------+
| tom   |   240 |
...
```sql
mysql> select sname,(ule+ula+uoa)/3 ave from score;
+-------+---------+
| sname | ave     |
+-------+---------+
| tom   | 80.0000 |
...
+-------------------+
4 rows in set (0.00 sec)
mysql> select concat(sno,' ',sname) from score;     //单引号中间是字符串
```
...
```sql
mysql> select sname,upper(sname) from score;
+-------+--------------+
| sname | upper(sname) |
+-------+--------------+
| tom   | TOM          |
...
```sql
mysql> select length('dadasdasfdswdredgcdf');
+--------------------------------+
| length('dadasdasfdswdredgcdf') |
+--------------------------------+
|                             20 |
+--------------------------------+
