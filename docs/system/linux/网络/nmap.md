...
```bash
Nmap（“网络映射器”）是一个用于网络探索和安全审计的开源工具。它被设计用于快速扫描大型网络，尽管它对单个主机运行良好。Nmap以新颖的方式使用原始IP数据包来确定网络上可用的主机、这些主机提供的服务（应用程序名称和版本）、运行的操作系统（和操作系统版本）、使用的数据包过滤器/防火墙类型以及数十种其他特征。虽然Nmap通常用于安全审计，但许多系统和网络管理员发现它对日常任务很有用，如网络清单、管理服务升级计划以及监控主机或服务正常运行时间。  

Nmap的输出是扫描目标的列表，根据使用的选项，每个目标都有补充信息。这些信息中的关键是“有趣的端口表”。该表列出了端口号和协议、服务名称和状态。状态为打开、过滤、关闭或未过滤。打开表示目标计算机上的应用程序正在侦听该端口上的连接/数据包。已筛选表示防火墙、筛选器或其他网络障碍正在阻塞该端口，因此Nmap无法判断该端口是打开还是关闭。关闭的端口没有应用程序在侦听，尽管它们可以随时打开。当端口对Nmap的探测做出响应时，它们被归类为未过滤端口，但Nmap无法确定它们是打开还是关闭。当Nmap无法确定两种状态中的哪一种描述端口时，它会报告打开|已筛选和关闭|已筛选的状态组合。端口表还可能包括以下情况下的软件版本详细信息

除了有趣的端口表之外，Nmap还可以提供有关目标的进一步信息，包括反向DNS名称、操作系统猜测、设备类型和MAC地址。

示例1中示出了典型的Nmap扫描。此示例中使用的唯一Nmap参数是-A，用于启用操作系统和版本检测、脚本扫描、-T4用于更快的执行；然后是主机名。
...
```
当运行无参数的Nmap时，将打印此选项摘要，并且最新版本始终位于https://svn.nmap.org/nmap/docs/nmap.usage.txt。它可以帮助人们记住最常见的选项，但不能替代本手册其余部分的深入文档。这里甚至不包括一些晦涩难懂的选项。
 ...
 目标 范围:
   ...
   -iL <输入文件名>: 主机地址、
   -iR <num hosts>: 在互联网上选择指定数目的随机目标进行随机扫描
   --exclude <host1[,host2][,host3],...>: 除去某些主机
   --excludefile <exclude_file>: 除去某些主机的列表
 主机 发现:
   -sL: 列表扫描-简单地列出要扫描的目标
   -sn: Ping扫描-禁用端口扫描
   -Pn: 将所有主机视为联机--跳过主机发现
...
## TARGET SPECIFICATION     [top](https://man7.org/linux/man-pages/man1/nmap.1.html#top_of_page)
...
```
       ...
       -iR num hosts (Choose random targets)
           对于互联网范围的调查和其他研究，您可能希望随机选择目标。num-hosts参数告诉Nmap要生成多少IP。不受欢迎的IP，例如某些专用、多播或未分配地址范围内的IP，会自动跳过。可以为无休止扫描指定参数0。请记住，一些网络管理员对未经授权的网络扫描感到愤怒，可能会抱怨
           nmap -Pn -sS -p 80 -iR 0 随机打开的网络服务器进行浏览 
...
任何网络侦察任务的第一步都是将一组（有时是巨大的）IP范围缩减为活动或感兴趣的主机列表。扫描每个IP地址的每个端口都很慢，而且通常没有必要。当然，让主机感兴趣的因素在很大程度上取决于扫描目的。网络管理员可能只对运行特定服务的主机感兴趣，而安全审计员可能会关心每个具有IP地址的设备。管理员可能只需使用ICMP ping就可以轻松定位其内部网络上的主机，而外部渗透测试人员可能会使用数十个不同的探测器来试图避开防火墙限制。

由于主机发现需求非常多样化，Nmap提供了各种各样的选项来定制所使用的技术。主机发现有时被称为ping扫描，但它远远超出了与无处不在的ping工具相关联的简单ICMP回显请求包。用户可以通过列表扫描（-sL）或禁用主机发现（-Pn）完全跳过发现步骤，或者通过多端口TCP SYN/ACK、UDP、SCTP INIT和ICMP探测的任意组合接入网络。这些探测的目的是寻求响应，以证明IP地址实际上是活动的（正被主机或网络设备使用）。在许多网络上，在任何给定的时间，只有一小部分IP地址处于活动状态。这在诸如10.0.0.0/8之类的专用地址空间中尤其常见。这个网络有1600万个IP，但我见过只有不到1000台机器的公司使用它。主机发现可以在稀疏分配的IP地址海洋中找到这些机器。

如果没有给出主机发现选项，Nmap将发送ICMP回显请求、TCP SYN数据包到端口443、TCP ACK数据包到港口80以及ICMP时间戳请求。（对于IPv6，ICMP时间戳请求被省略，因为它不是ICMPv6的一部分。）这些默认值相当于-PE-PS443-PA80-PP选项。例外情况是ARP（用于IPv4）和邻居发现（用于IPv6）扫描，它们用于本地以太网上的任何目标。对于没有特权的Unix shell用户，默认探测是使用连接系统调用到端口80和443的SYN数据包。在扫描本地网络时，这种主机发现通常就足够了，但建议使用一组更全面的发现探测器进行安全审核。

可以组合-P*选项（选择ping类型）。您可以通过使用不同的TCP端口/标志和ICMP代码发送许多探测类型来增加穿透严格防火墙的几率。还要注意，ARP/邻居发现默认情况下是针对本地以太网上的目标执行的，即使您指定了其他-P*选项，因为它几乎总是更快、更有效。

默认情况下，Nmap进行主机发现，然后对其确定为联机的每个主机执行端口扫描。即使指定了UDP探测（-PU）等非默认主机发现类型，情况也是如此。阅读-sn选项以了解如何仅执行主机发现，或使用-Pn跳过主机发现并扫描所有目标地址的端口。以下选项控制主机发现：

####        -sL (List Scan)

列表扫描是主机发现的一种退化形式，它只列出指定网络的每个主机，而不向目标主机发送任何数据包。默认情况下，Nmap仍然会对主机进行反向DNS解析以了解其名称。简单的主机名所提供的有用信息之多常常令人惊讶。例如，fw.chi是一家公司的芝加哥防火墙的名称。Nmap还报告了末尾的IP地址总数。列表扫描是一个很好的健全性检查，以确保您的目标有合适的IP地址。如果主机上有你不认识的域名，那么值得进一步调查，以防止扫描错误的公司网络。由于其想法是简单地打印目标主机的列表，因此无法将端口扫描、操作系统检测或主机发现等更高级别功能的选项与此相结合。如果您希望在执行此类更高级别功能的同时禁用主机发现，请阅读-Pn（跳过主机发现）选项。

####        -sn (No port scan)

此选项告诉Nmap不要在主机发现后进行端口扫描，只打印出响应主机发现探测的可用主机。这通常被称为“ping扫描”，但您也可以请求运行traceroute和NSE主机脚本。默认情况下，这比列表扫描更具侵入性，并且通常可以用于相同的目的。它允许在不引起太多注意的情况下对目标网络进行轻型侦察。对于攻击者来说，知道有多少主机可用比对每个IP和主机名进行列表扫描所提供的列表更有价值。
系统管理员通常也觉得这个选项很有价值。它可以很容易地用于统计网络上的可用机器或监控服务器的可用性。这通常被称为ping扫描，并且比ping广播地址更可靠，因为许多主机不回复广播查询。
使用-sn完成的默认主机发现包括ICMP回显请求、到端口443的TCP SYN、到端口445的TCP SYN，
在以前的Nmap版本中，-sn被称为-sP。

####        -Pn (No ping)

此选项完全跳过主机发现阶段。通常，Nmap使用这个阶段来确定用于更重扫描的活动机器，并测量网络的速度。默认情况下，Nmap仅对发现已启动的主机执行大量探测，如端口扫描、版本检测或操作系统检测。使用-Pn禁用主机发现会导致Nmap尝试针对指定的每个目标IP地址执行请求的扫描功能。因此，如果在命令行上指定了/16大小的网络，则会扫描所有65536个IP地址。与列表扫描一样，会跳过正确的主机发现，但Nmap不会停止并打印目标列表，而是继续执行请求的功能，就好像每个目标IP都处于活动状态一样。使用默认的定时参数，这可能会导致扫描速度变慢。要跳过主机发现和端口扫描，同时仍然允许NSE运行，请同时使用Pn-sn这两个选项。对于本地以太网上的机器，ARP扫描仍将执行（除非指定了--disable ARP ping或--send ip），因为Nmap需要MAC地址来进一步扫描目标主机。在以前的Nmap版本中，-Pn是-P0和-Pn。

####        -PS port list (TCP SYN Ping)

此选项发送一个设置了SYN标志的空TCP数据包。默认目标端口为80（可在编译时通过更改nmap.h中的default_TCP_PROBE_port_SPEC进行配置）。可以指定备用端口作为参数。语法与-p相同，只是不允许使用像T:这样的端口类型说明符。例如-PS22和-PS225-80113105035000。请注意，-PS和端口列表之间不能有空格。如果指定了多个探测，它们将并行发送。SYN标志向远程系统表明您正试图建立连接。通常情况下，目标端口将关闭，并发送回RST（重置）数据包。如果端口恰好打开，目标将通过SYN/ACK TCP数据包进行响应，从而进行TCP三向握手的第二步。然后，运行Nmap的机器通过RST响应而不是发送ACK包来拆除新生连接，该ACK包将完成三方握手并建立完整连接。RST数据包是由运行Nmap的机器的内核响应于意外的SYN/ACK而发送的，而不是由Nmap本身发送的。Nmap并不关心端口是打开还是关闭。先前讨论的RST或SYN/ACK响应告诉Nmap主机是可用的并且是响应的。在Unix盒子上，通常只有特权用户root能够发送和接收原始TCP数据包。对于没有特权的用户，会自动采用一种变通方法，即针对每个目标端口启动连接系统调用。这具有向目标主机发送SYN数据包以尝试建立连接的效果。如果连接返回快速成功或ECONNREFUSED失败，则底层TCP堆栈必须已接收到SYN/ACK或RST，并且主机标记为可用。如果连接尝试一直挂起，直到达到超时，则主机被标记为关闭。

####        -PA port list (TCP ACK Ping)

 TCP ACK ping与刚才讨论的SYN ping非常相似。正如您可能猜测的那样，不同之处在于设置了TCP ACK标志而不是SYN标志。这样的ACK包声称是通过已建立的TCP连接确认数据，但不存在这样的连接。因此，远程主机应该始终使用RST数据包进行响应，并在此过程中公开其存在。-PA选项使用与SYN探测器（80）相同的默认端口，还可以采用相同格式的目标端口列表。如果没有特权的用户尝试这样做，将使用前面讨论的连接解决方法。这种解决方法并不完美，因为连接实际上发送的是SYN数据包，而不是ACK。同时提供SYN和ACK ping探测的原因是为了最大限度地提高绕过防火墙的机会。许多管理员配置路由器和其他简单的防火墙来阻止传入的SYN数据包，但那些目的地为公司网站或邮件服务器等公共服务的数据包除外。这样可以防止与组织的其他传入连接，同时允许用户无阻碍地与Internet建立传出连接。这种无状态方法占用防火墙/路由器上的资源很少，并且受到硬件和软件过滤器的广泛支持。Linux Netfilter/iptables防火墙软件提供了--syn便利选项来实现这种无状态方法。当这样的无状态防火墙规则到位时，SYN ping探测（-PS）在发送到关闭的目标端口时可能会被阻止。在这种情况下，ACK探测会穿透这些规则。另一种常见类型的防火墙使用有状态规则来丢弃意外的数据包。这一功能最初主要出现在高端防火墙上，但多年来变得越来越普遍。Linux Netfilter/iptables系统通过--state选项支持这一点，该选项根据连接状态对数据包进行分类。SYN探测更有可能针对这样的系统工作，因为意外的ACK数据包通常被识别为伪造并丢弃。解决这个难题的方法是通过指定-PS和-PA同时发送SYN和ACK探测。

####        -PU port list (UDP Ping)

另一个主机发现选项是UDP ping，它向给定的端口发送UDP数据包。对于大多数端口，数据包将是空的，尽管有些端口使用更可能引发响应的特定于协议的有效载荷。有效载荷与服务和版本检测中使用的探测相同，并在nmap服务探测文件中定义。数据包内容也可能受到--data、--data字符串和--data长度选项的影响。端口列表采用与前面讨论的-PS和-PA选项相同的格式。如果未指定端口，则默认值为40125。可以在编译时通过在nmap.h中更改default_UDP_PROBE_PORT_SPEC来配置此默认值。默认情况下使用非常不常见的端口，因为对于此特定的扫描类型，通常不希望发送到打开的端口。在命中目标机器上的关闭端口后，UDP探测应引发ICMP端口不可访问的数据包作为回报。这向Nmap表示机器已启动并可用。许多其他类型的ICMP错误，如主机/网络无法访问或超过TTL，都表明主机已关闭或无法访问。缺乏回应也是这样解释的。如果到达打开的端口，大多数服务只是忽略空数据包，并且无法返回任何响应。这就是为什么默认的探测端口是40125，它极不可能被使用。一些服务，如字符生成器（chargen）协议，将响应空的UDP数据包，从而向Nmap透露机器可用。这种扫描类型的主要优点是它绕过了只屏蔽TCP的防火墙和筛选器。例如，我曾经拥有一台Linksys BEFW11S4无线宽带路由器。默认情况下，该设备的外部接口过滤了所有TCP端口，但UDP探测仍会引发无法访问端口的消息，从而泄露设备。

#### 				-PY port list (SCTP INIT Ping)

此选项发送包含最小INIT块的SCTP数据包。默认目标端口为80（可在编译时通过更改nmap.h中的default_SCTP_PROBE_port_SPEC进行配置）。可以指定备用端口作为参数。语法与-p相同，不同之处在于不允许使用像S:这样的端口类型说明符。示例为-PY22和-PY22，80795060。请注意，-PY和端口列表之间不能有空格。如果指定了多个探测，它们将并行发送。INIT区块向远程系统建议您正在尝试建立关联。通常，目标端口将被关闭，并且ABORT区块将被发送回。如果端口恰好是打开的，那么目标将通过使用INIT-ACK块进行响应来进行SCTP四向握手的第二步。如果运行Nmap的机器有一个功能性的SCTP堆栈，那么它会通过用ABORT块响应而不是发送COOKIE-ECHO块来拆除新生的关联，这将是四方握手的下一步。ABORT数据包由运行Nmap的机器的内核发送，以响应意外的INIT-ACK，而不是由Nmap本身发送。Nmap并不关心端口是打开还是关闭。先前讨论的ABORT或INIT-ACK响应告诉Nmap主机可用且响应。在Unix盒子上，通常只有特权用户root能够发送和接收原始SCTP数据包。目前，没有特权的用户无法使用SCTP INIT Ping。

####        -PE; -PP; -PM (ICMP Ping Types)

除了前面讨论的不寻常的TCP、UDP和SCTP主机发现类型外，Nmap还可以发送无处不在的ping程序发送的标准数据包。Nmap向目标IP地址发送ICMP类型8（回显请求）数据包，期望从可用主机返回类型0（回显回复）。不幸的是，对于网络探索者来说，许多主机和防火墙现在会阻止这些数据包，而不是按照RFC 1122[2]的要求进行响应。出于这个原因，仅ICMP扫描很少能够足够可靠地对抗互联网上的未知目标。但对于监控内部网络的系统管理员来说，这可能是一种实用而高效的方法。使用-PE选项可以启用此回显请求行为。虽然回显请求是标准的ICMP ping查询，但Nmap并没有就此止步。ICMP标准（RFC 792[3]和RFC 950[4]）还将时间戳请求、信息请求和地址掩码请求分组分别指定为代码13、15和17。虽然这些查询的表面目的是了解地址掩码和当前时间等信息，但它们可以很容易地用于主机发现。回复系统已启动并可用。Nmap目前没有实现信息请求包，因为它们没有得到广泛支持。RFC 1122坚持认为“主机不应该实现这些消息”。时间戳和地址掩码查询可以分别使用-PP和-PM选项发送。时间戳应答（ICMP代码14）或地址掩码应答（代码18）公开了主机可用。当管理员特别阻止回显请求数据包，同时忘记其他ICMP查询可以用于相同目的时，这两个查询可能很有价值。

####        -PO protocol list (IP Protocol Ping)

较新的主机发现选项之一是IP协议ping，它发送IP数据包，并在其IP标头中设置指定的协议号。协议列表采用与前面讨论的TCP、UDP和SCTP主机发现选项中的端口列表相同的格式。如果未指定任何协议，则默认情况下发送ICMP（协议1）、IGMP（协议2）和IP-in-IP（协议4）的多个IP数据包。默认协议可以在编译时通过在nmap.h中更改default_PROTO_PROBE_PORT_SPEC来配置。注意，对于ICMP、IGMP、TCP（协议6）、UDP（协议17）和SCTP（协议132），数据包是用正确的协议头发送的，而其他协议在发送时除了IP头之外没有其他数据（除非指定了-data、-data string或-data length选项中的任何一个）。此主机发现方法查找使用与探测相同的协议的响应，或ICMP协议无法访问的消息，这些消息表示目标主机不支持给定的协议。任何一种类型的响应都表示目标主机处于活动状态。

####        --disable-arp-ping (No ARP or ND Ping)

Nmap通常对本地连接的以太网主机进行ARP或IPv6邻居发现（ND）发现，即使使用了其他主机发现选项，如-Pn或-PE。要禁用此隐式行为，请使用--disable arp ping选项。默认行为通常更快，但此选项在使用代理ARP的网络上很有用，在代理ARP中，路由器推测性地回复所有ARP请求，使每个目标看起来都是根据ARP扫描启动的。

####        --discovery-ignore-rst

在某些情况下，防火墙可能会欺骗TCP重置（RST）回复，以响应对未占用或不允许的地址的探测。由于Nmap通常认为RST回复是目标到达的证据，这可能会导致浪费时间扫描不在那里的目标。使用--discovery忽略rst将阻止Nmap在主机发现期间考虑这些回复。在这种情况下，您可能需要选择额外的主机发现选项，以确保不会错过目标。

####        --traceroute (Trace path to host)

使用来自扫描结果的信息在扫描后执行跟踪路由，以确定最有可能到达目标的端口和协议。它适用于除连接扫描（-sT）和空闲扫描（-sI）之外的所有扫描类型。所有轨迹都使用Nmap的动态时序模型，并并行执行。Traceroute的工作原理是发送TTL（生存时间）较低的数据包，试图从扫描仪和目标主机之间的中间跃点引出ICMP time Exceeded消息。标准的traceroute实现从TTL为1开始，并递增TTL直到到达目标主机。Nmap的traceroute从高TTL开始，然后递减TTL直到达到零。向后做可以让Nmap使用巧妙的缓存算法来加快多台主机上的跟踪。根据网络条件的不同，Nmap平均每台主机少发送5-10个数据包。如果扫描单个子网（即192.168.0.0/24），Nmap可能只需要向大多数主机发送两个数据包。

### 端口扫描基础知识

虽然多年来Nmap的功能不断发展，但它最初是一款高效的端口扫描仪，这仍然是它的核心功能。简单的命令nmap target扫描主机目标上的1000个TCP端口。虽然许多端口扫描仪传统上将所有端口归为打开或关闭状态，但Nmap要精细得多。它将端口分为六种状态：打开、关闭、已过滤、未过滤、打开|已过滤或关闭|已过滤。

这些状态不是端口本身的固有属性，而是描述Nmap如何看待它们。例如，来自与目标相同网络的Nmap扫描可以将端口135/tcp显示为打开，而同时使用来自互联网的相同选项的扫描可以将该端口显示为已过滤。

Nmap识别的六个端口状态

####        open

应用程序正在该端口上主动接受TCP连接、UDP数据报或SCTP关联。找到这些通常是端口扫描的主要目标。有安全意识的人知道，每个开放的港口都是攻击的途径。攻击者和笔测试人员希望利用开放的端口，而管理员则试图在不妨碍合法用户的情况下关闭或使用防火墙保护它们。开放端口对于非安全扫描也很有意思，因为它们显示了可在网络上使用的服务。

####        closed

关闭的端口是可访问的（它接收并响应Nmap探测数据包），但没有应用程序在其上侦听。它们有助于显示主机在IP地址上（主机发现或ping扫描），并作为操作系统检测的一部分。由于关闭的端口是可访问的，因此可能值得稍后进行扫描，以防某些端口打开。管理员可能需要考虑使用防火墙阻止此类端口。然后，它们将以过滤状态出现，下面将进行讨论。

####        filtered

Nmap无法确定端口是否打开，因为数据包筛选阻止其探测到达端口。筛选可能来自专用防火墙设备、路由器规则或基于主机的防火墙软件。这些端口挫败了攻击者，因为它们提供的信息太少了。有时，它们会以ICMP错误消息作为响应，如类型3代码13（无法到达目的地：管理禁止通信），但更常见的是，只需丢弃探测而不响应的过滤器。这迫使Nmap重试几次，以防由于网络拥塞而不是过滤而导致探测失败。这大大减慢了扫描速度。

####        unfiltered

未过滤状态意味着端口可以访问，但Nmap无法确定它是打开还是关闭。只有用于映射防火墙规则集的ACK扫描将端口分类为这种状态。使用其他扫描类型（如Window扫描、SYN扫描或FIN扫描）扫描未过滤的端口可能有助于确定端口是否打开。

####        open|filtered

当Nmap无法确定端口是打开的还是已筛选的时，它会将端口置于此状态。这种情况发生在打开的端口没有响应的扫描类型中。没有响应也可能意味着数据包过滤器丢弃了探针或它引发的任何响应。因此，Nmap无法确定端口是打开的还是正在被过滤。UDP、IP协议、FIN、NULL和Xmas扫描以这种方式对端口进行分类。

####        closed|filtered

当Nmap无法确定端口是否已关闭或已筛选时，会使用此状态。它仅用于IP ID空闲扫描。
...
```
作为一名从事汽车维修的新手，我可能会挣扎数小时，试图将我的基本工具（锤子、胶带、扳手等）安装到手头的任务中。当我惨败，把我的老爷车拖到一个真正的机修工那里时，他总是在一个巨大的工具箱里四处搜寻，直到拿出完美的小发明，这让工作看起来毫不费力。端口扫描的艺术也是类似的。专家们了解数十种扫描技术，并为给定的任务选择合适的一种（或组合）。另一方面，经验不足的用户和脚本孩子试图用默认的SYN扫描来解决所有问题。由于Nmap是免费的，掌握端口扫描的唯一障碍是知识。这无疑胜过了汽车界，在汽车界，你可能需要高超的技能才能确定你需要一个支柱弹簧压缩机，然后你仍然需要为此支付数千美元。

大多数扫描类型仅对特权用户可用。这是因为它们发送和接收原始数据包，这需要在Unix系统上进行root访问。建议在Windows上使用管理员帐户，尽管当Npcap已经加载到操作系统中时，Nmap有时适用于该平台上的无特权用户。1997年Nmap发布时，要求root权限是一个严重的限制，因为许多用户只能访问共享的shell帐户。现在，世界不同了。计算机更便宜，更多的人总是直接上网，桌面Unix系统（包括Linux和Mac OS X）也很普遍。现在可以使用Windows版本的Nmap，使其能够在更多的台式机上运行。由于所有这些原因，用户不太需要从有限的共享shell帐户运行Nmap。这是幸运的，因为特权选项使Nmap更加强大和灵活。

虽然Nmap试图产生准确的结果，但请记住，它的所有见解都是基于目标机器（或其前面的防火墙）返回的数据包。这样的主机可能不可信，并发送旨在混淆或误导Nmap的响应。更常见的是不符合RFC的主机，它们对Nmap探测没有做出应有的响应。FIN、NULL和圣诞节扫描特别容易受到此问题的影响。这些问题是特定于某些扫描类型的，因此在各个扫描类型条目中进行了讨论。

本节介绍了Nmap支持的十几种端口扫描技术。一次只能使用一种方法，除了UDP扫描（-sU）和SCTP扫描类型（-sY，-sZ）中的任何一种可以与TCP扫描类型中的任意一种组合之外。作为内存辅助，端口扫描类型选项的形式为-sC，其中C是扫描名称中的一个突出字符，通常是第一个。唯一的例外是不推荐使用的FTP反弹扫描（-b）。默认情况下，Nmap执行SYN扫描，但如果用户没有发送原始数据包的适当权限（需要Unix上的root访问权限），它会替代连接扫描。在本节列出的扫描中，无特权用户只能执行连接和FTP反弹扫描。

       -sS (TCP SYN scan)

SYN扫描是默认的也是最受欢迎的扫描选项，这是有充分理由的。它可以快速执行，在不受限制性防火墙阻碍的快速网络上每秒扫描数千个端口。它也相对不引人注目和隐蔽，因为它从不完成TCP连接。SYN扫描适用于任何兼容的TCP堆栈，而不是像Nmap的FIN/NULL/Xmas、Maimon和空闲扫描那样依赖于特定平台的特性。它还允许在打开、关闭和过滤状态之间进行清晰、可靠的区分。

这种技术通常被称为半开放扫描，因为您没有打开完整的TCP连接。您发送一个SYN数据包，就好像您要打开一个真正的连接，然后等待响应一样。SYN/ACK表示端口正在侦听（打开），而RST（重置）表示未侦听。如果在多次重新传输后未接收到响应，则将端口标记为已过滤。如果接收到ICMP不可访问错误（类型3，代码0、1、2、3、9、10或13），则端口也被标记为已过滤。如果接收到SYN数据包（没有ACK标志）作为响应，则该端口也被认为是打开的。这可能是由于一种极为罕见的TCP功能，即同时打开或分离握手连接（请参阅https://nmap.org/misc/split-handshake.pdf）。

       -sT (TCP connect scan)
当SYN扫描不是选项时，TCP连接扫描是默认的TCP扫描类型。当用户没有原始数据包权限时，就会出现这种情况。Nmap不是像大多数其他扫描类型那样写入原始数据包，而是要求底层操作系统通过发出连接系统调用来建立与目标机器和端口的连接。这与web浏览器、P2P客户端和大多数其他支持网络的应用程序用于建立连接的高级系统调用相同。它是被称为Berkeley Sockets API的编程接口的一部分。Nmap使用此API获取每次连接尝试的状态信息，而不是离线读取原始数据包响应。

当SYN扫描可用时，它通常是一个更好的选择。与原始数据包相比，Nmap对高级连接调用的控制较少，因此效率较低。系统调用完成到打开目标端口的连接，而不是像SYN扫描那样执行半打开重置。这不仅需要更长的时间和更多的数据包才能获得相同的信息，而且目标机器更有可能记录连接。一个不错的IDS也能检测到这两种情况，但大多数机器都没有这样的报警系统。当Nmap连接并关闭连接而不发送数据时，普通Unix系统上的许多服务都会在syslog中添加一条注释，有时还会添加一条神秘的错误消息。当这种情况发生时，服务崩溃真是可悲，尽管这种情况并不常见。如果管理员在其日志中看到来自单个系统的大量连接尝试，则应该知道她已被连接扫描。

       -sU (UDP scans)
虽然互联网上大多数流行的服务都是通过TCP协议运行的，但UDP[5]服务被广泛部署。DNS、SNMP和DHCP（注册端口53、161/162和67/68）是最常见的三种。由于UDP扫描通常比TCP更慢、更困难，一些安全审核人员会忽略这些端口。这是一个错误，因为可利用的UDP服务非常常见，攻击者当然不会忽视整个协议。幸运的是，Nmap可以帮助清点UDP端口。

UDP扫描使用-SU选项激活。它可以与TCP扫描类型相结合，例如SYN扫描（-ss），以检查协议。

UDP扫描通过向每个目标端口发送UDP数据包来工作。对于一些常见端口，如53和161，会发送特定于协议的有效负载以提高响应率，但对于大多数端口，除非指定了--data、--data字符串或--data长度选项，否则数据包为空。如果返回ICMP端口无法访问错误（类型3，代码3），则关闭该端口。其他ICMP无法访问的错误（类型3，代码0、1、2、9、10或13）将端口标记为已过滤。偶尔，服务会以UDP数据包进行响应，以证明它是开放的。如果在重新传输后没有接收到响应，则该端口被分类为打开|已过滤。这意味着端口可能是打开的，或者数据包过滤器可能阻塞了通信。版本检测（-sV）可以用来帮助区分真正打开的端口和过滤的端口。

UDP扫描的一大挑战是快速完成。打开和过滤的端口很少发送任何响应，让Nmap超时，然后进行重新传输，以防丢失探测或响应。关闭的港口往往是一个更大的问题。他们通常会发回ICMP端口无法访问的错误。但与关闭的TCP端口响应SYN或连接扫描发送的RST数据包不同，许多主机默认情况下会限制ICMP端口无法访问的消息。Linux和Solaris对此特别严格。例如，Linux 2.4.20内核将无法到达目的地的消息限制为每秒一条（在net/ipv4/icmp.c中）。
Nmap检测到速率限制并相应地降低速度，以避免目标机器丢弃的无用数据包充斥网络。不幸的是，Linux风格的每秒一个数据包的限制使得65536端口的扫描需要超过18个小时。加快UDP扫描速度的想法包括并行扫描更多主机，首先只对流行端口进行快速扫描，从防火墙后面进行扫描，以及使用主机超时跳过慢速主机。

       -sY (SCTP INIT scan)
SCTP[6]是TCP和UDP协议的一种相对较新的替代方案，它结合了TCP和UDP的大部分特性，还添加了多归属和多流等新功能。它主要用于SS7/SIGTRAN相关服务，但也有可能用于其他应用程序。SCTP INIT扫描是相当于TCP SYN扫描的SCTP。它可以快速执行，在不受限制性防火墙阻碍的快速网络上每秒扫描数千个端口。与SYN扫描一样，INIT扫描相对来说是不引人注目和隐秘的，因为它永远不会完成SCTP关联。它还允许在打开、关闭和过滤状态之间进行清晰、可靠的区分。

这种技术通常被称为半开放扫描，因为您没有打开完整的SCTP关联。您发送一个INIT区块，就好像您要打开一个真正的关联，然后等待响应一样。INIT-ACK块表示端口正在侦听（打开），而ABORT块表示非侦听器。如果在多次重新传输后未接收到响应，则将端口标记为已过滤。如果接收到ICMP不可访问错误（类型3，代码0、1、2、3、9、10或13），则端口也被标记为已过滤。

       -sN; -sF; -sX (TCP NULL, FIN, and Xmas scans)
这三种扫描类型（下一节中描述的--scanflags选项可能会有更多扫描类型）利用了TCP RFC[7]中的一个细微漏洞来区分开放端口和关闭端口。RFC 793的第65页说，“如果[目的地]端口状态为CLOSED……不包含RST的传入段会导致发送RST作为响应。”然后下一页讨论发送到未设置SYN、RST或ACK位的打开端口的数据包，声明：“你不太可能到达这里，但如果你到达了，请删除段，然后返回。”
当扫描符合此RFC文本的系统时，如果端口关闭，任何不包含SYN、RST或ACK位的数据包都将导致返回RST，如果端口打开，则根本没有响应。只要这三个比特都不包括，其他三个比特（FIN、PSH和URG）的任何组合都可以。Nmap利用这三种扫描类型：
...
仅设置TCP FIN位。
           ...
           Xmas scan (-sX)
设置FIN、PSH和URG标志，将数据包像圣诞树一样点亮。

除了探测数据包中设置的TCP标志外，这三种扫描类型在行为上完全相同。如果接收到RST数据包，则认为该端口已关闭，而没有响应则表示该端口已打开|已过滤。如果接收到ICMP无法访问错误（类型3，代码0、1、2、3、9、10或13），则端口被标记为已过滤。

这些扫描类型的关键优势在于，它们可以偷偷通过某些无状态防火墙和数据包过滤路由器。另一个优点是，这些扫描类型甚至比SYN扫描更隐蔽。尽管大多数现代IDS产品都可以配置为检测它们，但不要指望这一点。最大的缺点是并非所有系统都严格遵循RFC 793。许多系统向探测器发送RST响应，而不管端口是否打开。这将导致所有端口被标记为关闭。实现这一点的主要操作系统是Microsoft Windows、许多Cisco设备、BSDI和IBM OS/400。不过，这种扫描确实适用于大多数基于Unix的系统。这些扫描的另一个缺点是，它们无法区分打开的端口和某些已筛选的端口，从而使响应处于打开状态|已筛选状态。
...
## PORT SPECIFICATION AND SCAN ORDER     [top](https://man7.org/linux/man-pages/man1/nmap.1.html#top_of_page)
...
## SERVICE AND VERSION DETECTION     [top](https://man7.org/linux/man-pages/man1/nmap.1.html#top_of_page)
...
## OS DETECTION     [top](https://man7.org/linux/man-pages/man1/nmap.1.html#top_of_page)
...
## NMAP SCRIPTING ENGINE (NSE)     [top](https://man7.org/linux/man-pages/man1/nmap.1.html#top_of_page)
...
## TIMING AND PERFORMANCE     [top](https://man7.org/linux/man-pages/man1/nmap.1.html#top_of_page)
...
```
       ...
       template)
           ...
           --initial-rtt-timeout 500ms --max-retries 6 and sets the
           ...
           15m --script-timeout 10m as well as setting the maximum TCP
           and SCTP scan delay to 5ms. Maximum UDP scan delay is not set
           by T4 or T5, but it can be set with the --max-scan-delay
           option.
```

## FIREWALL/IDS EVASION AND SPOOFING     [top](https://man7.org/linux/man-pages/man1/nmap.1.html#top_of_page)
...
```
       ...
       --ip-options R|S [route]|L [route]|T|U ... ; --ip-options hex
...
## OUTPUT     [top](https://man7.org/linux/man-pages/man1/nmap.1.html#top_of_page)
...
## MISCELLANEOUS OPTIONS     [top](https://man7.org/linux/man-pages/man1/nmap.1.html#top_of_page)
...
## RUNTIME INTERACTION     [top](https://man7.org/linux/man-pages/man1/nmap.1.html#top_of_page)
...
## EXAMPLES     [top](https://man7.org/linux/man-pages/man1/nmap.1.html#top_of_page)
...
## NMAP BOOK     [top](https://man7.org/linux/man-pages/man1/nmap.1.html#top_of_page)
...
里昂戈登<fyodor@nmap.org>1997年编写并发布了Nmap。从那时起，数百人做出了宝贵的贡献，如Nmap分发的CHANGELOG文件中所述，也可从https://nmap.org/changelog.html。大卫·费菲尔德和丹尼尔·米勒多年来的巨大贡献值得特别表彰！
